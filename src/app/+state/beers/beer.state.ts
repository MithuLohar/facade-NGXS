...
export class BeersState {
  ...
  private persistLoad(
    ctx: StateContext<Beers[]>,
    { beers }: PersistBeersLoadAction
  ) {
    ctx.setState(beers);
  }
  ...
  private persistDelete(
    ctx: StateContext<Beers[]>,
    { payload }: PersistBeersDeleteAction
  ) {
    ctx.setState(
      ctx.getState().filter((beer: Beers) => !payload.includes(beer))
    );
  }
  ...
  private persistEdit(
    ctx: StateContext<Beers[]>,
    { payload }: PersistBeersEditAction
  ) {
    ctx.setState(
      ctx.getState().reduce((acc, cur) => {
